...
def seed_pincodes_endpoint(db: Session = Depends(get_db)):
    ...
    # Run synchronously to capture output and avoid path/python executable issues
    try:
        import sys
        result = subprocess.run([sys.executable, script_path], capture_output=True, text=True, cwd=os.path.dirname(script_path))
        if result.returncode != 0:
            return {"success": False, "error": "Script failed", "stderr": result.stderr}
        return {"success": True, "message": "Seeding completed", "stdout": result.stdout}
