      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-[var(--text-muted)] mt-2 max-w-2xl">
          ...
          <div className="relative shrink-0">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2 w-full md:w-64 lg:w-80 bg-slate-50 dark:bg-[var(--card)] border border-[var(--border)] rounded-xl text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--primary)]/40 focus:bg-white dark:focus:bg-slate-950"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && !isLoading && (
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 rounded-2xl p-8 flex flex-col items-center justify-center text-center mb-8">
            ...
            <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2">We encountered an issue</h2>
            <p className="text-slate-600 dark:text-[var(--text-muted)] max-w-md">
              {error}
            </p>
            <button 
        ...
        {!error && !isLoading && continueCourse && (
          <div className="mb-10 premium-card p-6 flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-grow w-full">
              ...
              <h3 className="text-xl font-bold text-[var(--text-primary)] mb-2">{continueCourse.title}</h3>
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-medium text-slate-600 dark:text-[var(--text-muted)] whitespace-nowrap">{continueCourse.progress}% Completed</span>
              </div>
            </div>
          ...
          <div className="flex overflow-x-auto pb-4 mb-8 gap-2 scrollbar-hide">
            {categories.map(cat => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${ activeTab === cat.id ? "bg-[#FF9933] text-white" : "bg-[var(--card)] text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:bg-[#1F2937] dark:hover:bg-[var(--card-elevated)] border border-slate-200 dark:border-slate-800" }`}
              >
                {cat.label}
              </button>
            ))}
          ...
          <>
            {filteredCourses.length > 0 ? (
                ...
                {filteredCourses.map(course => (
                  <div key={course.id} className="premium-card premium-card-hover group flex flex-col h-full overflow-hidden">
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm text-slate-600 dark:text-[var(--text-muted)] mb-5">
                        <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" /> {course.duration}</span>
                        <span className="flex items-center gap-1.5"><BookOpen className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" /> {course.lessons} lessons</span>
                        <span className="flex items-center gap-1.5"><Activity className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" /> {course.readingTime}</span>
                        <span className="flex items-center gap-1.5"><Star className="w-4 h-4 text-amber-500 fill-current" /> {course.rating}</span>
                      </div>

                      <div className="mb-5">
                        <h4 className="text-xs font-semibold text-[var(--text-primary)] uppercase tracking-wider mb-2.5">Key Points</h4>
                        <ul className="text-sm text-slate-600 dark:text-[var(--text-muted)] space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 text-sm text-slate-600 dark:text-[var(--text-muted)] mb-6 mt-auto">
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="mb-5">
                          <div className="flex justify-between text-xs mb-1.5">
                            <span className="text-slate-600 dark:text-[var(--text-muted)] font-medium">Progress</span>
                            <span className="font-semibold text-[var(--text-primary)]">{course.progress}%</span>
                          </div>
                        ...
                        <Link
                          ...
                          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/80 text-[var(--text-primary)] font-medium hover:bg-slate-100 dark:bg-[#1F2937] dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700"
                        >
                          {course.progress === 100 ? (
              ...
              <div className="premium-card text-center py-16">
                ...
                <h3 className="text-lg font-medium text-[var(--text-primary)] mb-2">No courses found</h3>
                <p className="text-[var(--text-muted)]">We couldn&apos;t find any courses matching your search criteria.</p>
                <button 
