...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const fileItem = vaultItems.find(item => item.id === params.id) || {
      filename: "Legal_Document.pdf",
      vault_category: "Evidence",
      file_type: "application/pdf"
    };

    const docName = fileItem.filename;
...
Return ONLY a raw JSON object with NO markdown, NO backticks:
{
  ...
  "applicable_laws": ["Act 1", "Act 2"]
              ...
              {
                role: "user",
                content: `Analyze document: ${docName} (Category: ${fileItem.vault_category})`
              }
            ]
          })
        });
        ...
        console.warn("Groq Vault Analyze failed, fallback to smart OCR analysis:", err);
      }
    }
