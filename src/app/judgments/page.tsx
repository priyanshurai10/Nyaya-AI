...
export default function LandmarkJudgmentsPage() {
  ...
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    fetch(`https://nyaya-ai-backend-tyy5.onrender.com/api/v1/judgments`)
      .then(res => {
        if (!res.ok) {
    ...
    <div className="min-h-screen bg-[var(--background)] p-6 lg:p-8">
      ...
      {error && !isLoading && (
        <div className="max-w-6xl mx-auto mb-8 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/50 rounded-xl p-8 flex flex-col items-center justify-center text-center gap-3">
          <AlertTriangle className="w-12 h-12 text-red-600 dark:text-[var(--danger)] shrink-0 mb-2" />
          <div className="text-xl font-bold text-red-800 dark:text-red-300">
            {error}
          </div>
          <p className="text-base text-red-700 dark:text-[var(--danger)] max-w-md">
        ...
        <div className="max-w-6xl mx-auto mb-10 flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-[var(--card)] text-[var(--text-primary)] focus:ring-2 focus:ring-[#FF9933] focus:border-transparent outline-none transition-all"
            />
          </div>
          <div className="relative md:w-64">
            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-[var(--text-muted)]" />
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="w-full pl-10 pr-10 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-[var(--card)] text-[var(--text-primary)] focus:ring-2 focus:ring-[#FF9933] focus:border-transparent outline-none appearance-none transition-all"
            >
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
              <svg className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center p-20 bg-[var(--card)] rounded-3xl border border-slate-200 dark:border-slate-800 text-[var(--text-muted)]">
            <div className="w-12 h-12 border-4 border-[var(--border)] border-t-[#FF9933] rounded-full animate-spin mb-6"></div>
            <p className="text-lg font-medium">Loading landmark judgments...</p>
          </div>
        ) : error ? null : filteredJudgments.length === 0 ? (
          <div className="text-center p-12 bg-[var(--card)] rounded-3xl border border-slate-200 dark:border-slate-800 text-[var(--text-muted)]">
            No judgments found matching your search.
          </div>
        ) : filteredJudgments.map(case_ => (
          <div key={case_.id} className="bg-[var(--card)] rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
            <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-800/50 flex flex-col md:flex-row md:items-start justify-between gap-6">
              ...
              <div className="flex-1">
                ...
                <h2 className="text-2xl md:text-3xl font-black text-[var(--text-primary)] mb-4 leading-tight">
                  {case_.name}
                </h2>
                
                <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)]">
              ...
              <button 
                ...
                className={`shrink-0 p-3 rounded-full transition-colors ${bookmarked[case_.id] ? 'bg-[var(--primary-subtle)] text-indigo-600 dark:bg-[var(--primary-subtle)]0/10 dark:text-[var(--primary)]' : 'hover:bg-slate-100 dark:bg-[#1F2937] dark:hover:bg-[var(--card-elevated)] text-slate-400 hover:text-[var(--text-primary)] dark:hover:text-white'}`}
