...
export default function TrustPrivacyPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1220] p-6 lg:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400 dark:text-slate-500 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {policies.map((policy, idx) => (
            <div key={idx} className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800">
              <policy.icon className="w-8 h-8 text-[#FF9933] mb-4" />
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-3">
                {policy.title}
              </h3>
              <p className="text-slate-600 dark:text-slate-400 dark:text-slate-500 leading-relaxed">
                {policy.desc}
              </p>
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
            {[
              ...
              <div key={idx} className="p-6 hover:bg-slate-50 dark:bg-[#0B1220] dark:hover:bg-slate-800/30 transition-colors flex items-center justify-between cursor-pointer group">
                <span className="font-medium text-slate-700 dark:text-slate-300 group-hover:text-[#FF9933] transition-colors">
                  {doc}
                </span>
                <span className="text-sm font-medium text-blue-600 dark:text-blue-400">View Document</span>
              </div>
            ))}
          </div>
        </div>
