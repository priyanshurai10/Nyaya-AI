...
export default function EvidenceVaultPage() {
  ...
  const fetchFiles = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nyaya-ai-backend-tyy5.onrender.com'}/api/v1/vault`);
  ...
  const handleUpload = async (fileList: FileList | null) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nyaya-ai-backend-tyy5.onrender.com'}/api/v1/vault/upload`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nyaya-ai-backend-tyy5.onrender.com'}/api/v1/vault/${id}`, { method: "DELETE" });
  ...
  const handleAnalyze = async (file: VaultFile) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nyaya-ai-backend-tyy5.onrender.com'}/api/v1/vault/analyze/${file.id}`, {
        method: "POST",
      });
      const data = await res.json();
