...
export default function LegalKnowledgeCenterPage() {
  ...
  const toggleBookmark = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://nyaya-ai-backend-tyy5.onrender.com'}/api/v1/user/bookmarks`, {
        method: 'POST',
      ...
      <div className="max-w-7xl mx-auto mb-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-[var(--text-muted)]" />
            <input 
              type="text" 
              placeholder="Search laws, rights, or articles..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 pr-4 py-3 w-full md:w-80 bg-[var(--card)] border border-slate-200 dark:border-slate-800 rounded-xl text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-[#FF9933]"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="space-y-2">
            <h3 className="font-bold text-[var(--text-primary)] mb-4 uppercase text-xs tracking-wider">Browse by Act</h3>
            {categories.map(cat => (
              <button
                key={cat.id}
                onClick={() => {
                ...
                className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-all flex items-center justify-between ${ activeCategory === cat.id ? "bg-[#FF9933] text-white shadow-md" : "bg-transparent text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)] hover:bg-slate-200 dark:hover:bg-[var(--card-elevated)]" }`}
              >
          ...
          {allTags.length > 0 && (
            <div className="bg-[var(--card)] p-5 rounded-2xl border border-slate-200 dark:border-slate-800">
              <h3 className="font-bold text-[var(--text-primary)] mb-4 uppercase text-xs tracking-wider flex items-center justify-between">
                Filter by Tags
                {activeTag && (
                  <button 
              ...
              <div className="flex flex-wrap gap-2">
                {allTags.map(tag => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all border ${ activeTag === tag ? "bg-[#FF9933] text-white border-[#FF9933]" : "bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)] border-slate-200 dark:border-slate-700 hover:border-[#FF9933] hover:text-[#FF9933]" }`}
                  >
                    {tag}
                  </button>
                ))}
          ...
          <div className="bg-blue-50 bg-[var(--primary-subtle)] p-5 rounded-2xl border border-blue-100 dark:border-blue-900/30">
            <Info className="w-6 h-6 text-blue-600 dark:text-[var(--primary)] mb-3" />
            <h4 className="font-bold text-blue-900 dark:text-blue-300 text-sm mb-2">Educational Content</h4>
            <p className="text-xs text-blue-700/80 dark:text-[var(--primary)]/80 leading-relaxed">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {!isLoading && isError && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/30 p-8 rounded-3xl text-center">
              ...
              <h3 className="text-lg font-bold text-[var(--text-primary)] mb-2">Unable to load Knowledge Center</h3>
              <p className="text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)]">There was a problem fetching the data. Please try again later.</p>
              <button 
          ...
          {!isLoading && !isError && filteredTopics.map(topic => {
            ...
            return (
              <div key={topic.id} className={`bg-[var(--card)] rounded-3xl border ${isExpanded ? 'border-[#FF9933]' : 'border-slate-200 dark:border-slate-800'} overflow-hidden shadow-sm hover:shadow-md transition-all duration-300`}>
                <div 
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      {topic.tags?.slice(0, 3).map((tag: string) => (
                        <span key={tag} className="text-[10px] font-semibold text-[var(--text-muted)] dark:text-[var(--text-muted)] border border-slate-200 dark:border-slate-700 px-2 py-1 rounded-md hidden sm:inline-block">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <h2 className="text-xl md:text-2xl font-bold text-[var(--text-primary)] group-hover:text-[#FF9933] transition-colors">
                      {topic.title}
                    </h2>
                    
                    {!isExpanded && (
                      <p className="mt-3 text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)] line-clamp-2 text-sm md:text-base">
                  ...
                  <div className="flex items-center justify-end gap-3 shrink-0">
                    <button 
                      ...
                      className="p-2 rounded-full hover:bg-slate-100 dark:bg-[#1F2937] dark:hover:bg-[var(--card-elevated)] transition-colors"
                    >
                      {isBookmarked ? (
                        <BookmarkCheck className="w-6 h-6 text-[#FF9933]" />
                      ) : (
                        <Bookmark className="w-6 h-6 text-slate-400 dark:text-[var(--text-muted)]" />
                      )}
                    </button>
                    <div className="w-10 h-10 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center border border-slate-100 dark:border-slate-700">
                      {isExpanded ? <ChevronDown className="w-5 h-5 text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)]" /> : <ChevronRight className="w-5 h-5 text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)]" />}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="px-6 pb-6 md:px-8 md:pb-8 border-t border-slate-100 dark:border-slate-800/50 pt-6">
                    {topic.simple_explanation && (
                      <div className="mb-8 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 p-5 rounded-2xl flex items-start gap-4">
                        <Lightbulb className="w-6 h-6 text-blue-600 dark:text-[var(--primary)] shrink-0 mt-1" />
                        <div>
                    ...
                    <div className="prose dark:prose-invert max-w-none">
                      ...
                      {topic.example && (
                        <div className="mb-8 bg-slate-50 dark:bg-[#1E293B] p-5 rounded-xl border border-slate-100 dark:border-slate-700">
                          <h4 className="font-bold text-[var(--text-primary)] mb-2">Example / Scenario</h4>
                          ...
                          <div className="space-y-4">
                            {topic.faqs.map((faq: any, idx: number) => (
                              <div key={idx} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 border border-slate-100 dark:border-slate-700/50">
                                <p className="font-bold text-[var(--text-primary)] text-sm mb-2">{faq.question}</p>
                                <p className="text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)] text-sm leading-relaxed">{faq.answer}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {topic.related_laws && topic.related_laws.length > 0 && (
                        <div className="mb-6">
                          <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2 uppercase tracking-wider">
                            <LinkIcon className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" />
                            Related Laws
                          </h3>
                          <div className="flex flex-wrap gap-2">
                      ...
                      {topic.related_judgments && topic.related_judgments.length > 0 && (
                        <div>
                          <h3 className="text-sm font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2 uppercase tracking-wider">
                            <Book className="w-4 h-4 text-slate-400 dark:text-[var(--text-muted)]" />
                            Related Judgments
                          </h3>
                          <ul className="list-disc list-inside space-y-1 text-sm text-slate-600 dark:text-[var(--text-muted)] dark:text-[var(--text-muted)]">
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800/50 flex flex-wrap gap-2">
                       {topic.tags?.map((tag: string) => (
                          <button 
                            ...
                            className="flex items-center gap-1.5 text-xs font-semibold text-[var(--text-muted)] dark:text-[var(--text-muted)] bg-slate-50 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-full transition-colors"
                          >
          ...
          {!isLoading && !isError && filteredTopics.length === 0 && (
            <div className="text-center py-20 bg-[var(--card)] border border-slate-200 dark:border-slate-800 rounded-3xl">
              <Book className="w-12 h-12 text-slate-300 dark:text-[var(--text-muted)] mx-auto mb-4" />
              <h3 className="text-lg font-bold text-[var(--text-primary)]">No topics found</h3>
