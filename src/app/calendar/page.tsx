...
import { Calendar as CalendarIcon, Clock, MapPin, Video, User, Plus, ChevronLeft, ChevronRight, AlertCircle, X, Check, Trash2 } from "lucide-react";

const EVENT_COLORS: Record<string, string> = {
  hearing: "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400",
  filing: "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400",
  notice_deadline: "bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400",
  rti_deadline: "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400",
...
export default function LitigationCalendarPage() {
  ...
  const [addForm, setAddForm] = useState({ title: "", event_type: "hearing", event_date: "" });
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      if (data.success) {
        setAddSuccess(true);
        setAddForm({ title: "", event_type: "hearing", event_date: "" });
        await fetchEvents();
        setTimeout(() => {
  ...
  // Build set of event dates for highlighting
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-600 dark:text-[var(--text-muted)] mt-2">
              Track court hearings, filing deadlines, RTI replies, and legal consultations.
            </p>
          </div>
          <button
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
                  ...
                  <div
                    ...
                    className={`py-1.5 rounded-lg text-xs relative cursor-default ${
                      isToday
                        ...
                        : hasEvent
                        ? "bg-[#138808]/10 text-[#138808] dark:text-emerald-400 font-semibold"
                        : "text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
                    }`}
                  >
                    {day}
                    {hasEvent && !isToday && (
                      <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#138808] dark:bg-emerald-400" />
                    )}
                  </div>
                );
              })}
          ...
          <div className="bg-gradient-to-br from-[#138808] to-green-700 rounded-2xl p-6 text-white shadow-md">
            ...
            <button
              onClick={() => alert("WhatsApp reminder integration coming in next update!")}
              className="px-4 py-2 bg-white text-green-700 text-sm font-bold rounded-lg shadow-sm hover:bg-green-50 w-full transition-colors"
            >
              Enable Reminders
            </button>
          </div>
          ...
          <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-5 text-white">
            <div className="text-[10px] font-black uppercase tracking-widest text-indigo-200 mb-2">🔮 Next Update</div>
            <h3 className="font-bold mb-2">Google Calendar Sync</h3>
            <p className="text-white/70 text-xs leading-relaxed">
              Sync your legal hearings to Google Calendar and get cross-device alerts automatically.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex bg-[var(--card)] rounded-xl p-1 border border-slate-200 dark:border-slate-800 w-fit mb-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === "upcoming" ? "bg-slate-100 dark:bg-slate-800 text-[var(--text-primary)]" : "text-slate-600 dark:text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
            >
              Upcoming
            </button>
            <button
              onClick={() => setView("past")}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${view === "past" ? "bg-slate-100 dark:bg-slate-800 text-[var(--text-primary)]" : "text-slate-600 dark:text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
            >
              Past Events
            </button>
          </div>
          ...
          {isLoading ? (
            <div className="bg-[var(--card)] border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center">
              ...
              <p className="text-[var(--text-muted)] text-sm">Loading calendar events...</p>
            </div>
          ) : filteredEvents.length === 0 ? (
            <div className="bg-[var(--card)] border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center">
              <CalendarIcon className="w-12 h-12 text-slate-300 mx-auto mb-4" />
              <h3 className="font-semibold text-[var(--text-primary)] mb-1">No {view} events</h3>
              <p className="text-[var(--text-muted)] text-sm">Click "Add Event" to schedule a hearing or deadline.</p>
            </div>
          ) : (
            filteredEvents.map((event) => (
              <div key={event.id} className="bg-[var(--card)] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:shadow-md transition-shadow flex gap-4 group">
                ...
                <div className="hidden sm:flex flex-col items-center justify-center w-20 shrink-0 border-r border-slate-100 dark:border-slate-800 pr-4 text-center">
                  ...
                  <span className="text-[10px] text-[#FF9933] font-semibold mt-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${EVENT_COLORS[event.type] || "bg-slate-100 text-slate-600"}`}>
                      {EVENT_LABELS[event.type] || event.type}
                    </span>
                  </div>
                  <h3 className="text-base font-bold text-[var(--text-primary)] mb-2 leading-snug">{event.title}</h3>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <div className="bg-[var(--card)] rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 dark:border-slate-700" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-[var(--text-primary)]">Add New Event</h2>
            ...
            {addSuccess ? (
              <div className="text-center py-8">
                ...
                <p className="font-semibold text-[var(--text-primary)]">Event Added!</p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-2">Event Type *</label>
                  <select
                  ...
                  >
                    ...
                    <option value="rti_deadline">RTI Deadline</option>
