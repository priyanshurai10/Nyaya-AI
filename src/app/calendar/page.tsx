...
export default function LitigationCalendarPage() {
  ...
  const fetchEvents = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`https://nyaya-ai-backend-tyy5.onrender.com/api/v1/calendar`);
  ...
  const handleDeleteEvent = async (id: string) => {
    try {
      await fetch(`https://nyaya-ai-backend-tyy5.onrender.com/api/v1/calendar/${id}`, { method: "DELETE" });
