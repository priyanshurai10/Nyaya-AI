...
export default function TalkToSeniorSpecialistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1/consultation/payment/submit`, {
        method: "POST",
        body: submitData
      });
      ...
      const verifyRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1/consultation/payment/verify/${txId}`, {
        method: "POST",
        body: verifyData
      });
